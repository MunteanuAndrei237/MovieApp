...
const changeFavouriteThunk = createAsyncThunk(
  "favourites/changeFavouriteThunk",
  async ({ movieId, favouriteState }, { getState }) => {
    try {
      const url =
      ...
      alert(
        "Error: Couldn't " +
          (favouriteState === false ? "add" : "remove") +
          " movie " +
          (favouriteState === false ? "to" : "from") +
          " favourites",
      );
...
const fetchFavouriteMoviesThunk = createAsyncThunk(
  "/favourite/fetchFavouriteMoviesThunk",
  async (_, { getState }) => {
    try {
      const url =
        "https://api.themoviedb.org/3/account/" +
        getState().user.user.id +
        "/favorite/movies?language=en-US&page=" +
        getState().favourites.favouritesPage +
...
const favouritesSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(fetchFavouriteMoviesThunk.fulfilled, (state, action) => {
      ...
      else state.favouritesStatus = "fetchMore";
      state.favouritesMovies = state.favouritesMovies.concat(
...
export { changeFavouriteThunk, fetchFavouriteMoviesThunk };
